...
## Project Overview
Restaurant management system built with Next.js, TypeScript, Supabase, and Tailwind CSS. Features order management (Kanban), kitchen display system (KDS), inventory control, productivity tracking, technical sheets, and label printing.
...
- **API Routes**:
  - `/api/export/[tabela]` for export CSV/XLSX
  - `/api/import/[tabela]` for import CSV/XLSX
  - `/api/print/[type]/[id]` for printing (thermal/PDF/HTML)
- **Real-time Subscriptions**:
  - `supabase.channel('table_changes').on('postgres_changes', { event: '*', schema: 'public', table: 'table_name' }, callback).subscribe()`
...
- **Import/Export**:
  - Export: `/api/export/[table]?formato=csv&dataInicio=...&dataFim=...`
  - Import: POST `/api/import/[table]` with `multipart/form-data`
...
- **Real-time Updates**:
  - `supabase.channel('pedidos_changes').on('postgres_changes', { event: '*', schema: 'public', table: 'pedidos' }, (payload) => { /* handle update */ }).subscribe()`
