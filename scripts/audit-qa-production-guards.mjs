...
for (const file of files) {
  ...
  if (rel.startsWith('.github/workflows/') && content.includes(productionRef) && /(e2e|smoke|qa)/i.test(content)) {
  ...
  if (touchesSupabase && mayMutate && !content.includes(requiredGuard)) {
    findings.push(`${rel}: mutation-capable QA/E2E script uses Supabase credentials without the shared safe-target guard.`);
  }
}
