...
export function assertSafeQaWriteTarget({
  ...
  if (!resolvedRef) {
    throw new Error('QA write guard: unable to resolve Supabase project ref. Refusing mutation.');
  }

  if (resolvedRef === PRODUCTION_PROJECT_REF) {
    throw new Error(
      `QA write guard: production project ${PRODUCTION_PROJECT_REF} is immutable for QA/E2E automation.`,
    );
  }

  if (resolvedRef !== STAGING_PROJECT_REF) {
    throw new Error(
      `QA write guard: mutations are allowed only on gestify-staging (${STAGING_PROJECT_REF}); got ${resolvedRef}.`,
    );
  }

  if (allowWrites !== 'true') {
    throw new Error(
      'QA write guard: set GESTIFY_QA_ALLOW_WRITES=true explicitly for staging-only mutation tests.',
    );
  }
