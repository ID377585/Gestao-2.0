...
function Top3BarChart({ items }: { items: { label: string; value: number }[] }) {
  ...
  return (
    <div className="space-y-2">
      {items.map((it, idx) => (
        <div key={it.label} className="flex items-start gap-3 sm:items-center">
          <div className="w-6 shrink-0 pt-0.5 text-xs text-muted-foreground sm:pt-0">
            #{idx + 1}
          </div>

          <div className="min-w-0 flex-1">
            <div className="mb-1 flex items-start justify-between gap-3">
              <div className="min-w-0 text-sm break-words sm:truncate">
                {it.label}
              </div>
...
export default async function ProdutividadePage() {
  ...
  for (const o of safeOrders) {
    timesByOrder.set(o.id, { createdAt: new Date(o.created_at) });
  }
  ...
  let somaMinutos = 0;
  let qtdComTempo = 0;
  let somaMinutosProd = 0;
  let qtdComTempoProd = 0;

  for (const [, t] of timesByOrder) {
    if (t.deliveredAt) {
      const diffMin =
        (t.deliveredAt.getTime() - t.createdAt.getTime()) / (1000 * 60);
      if (diffMin > 0) {
        somaMinutos += diffMin;
        qtdComTempo++;
      }
    ...
    if (t.acceptedAt && t.separatedAt) {
      const diffMin =
        (t.separatedAt.getTime() - t.acceptedAt.getTime()) / (1000 * 60);
      if (diffMin > 0) {
        somaMinutosProd += diffMin;
        qtdComTempoProd++;
      }
    }
  }
  ...
  const tempoMedioProducaoMin =
    qtdComTempoProd > 0 ? somaMinutosProd / qtdComTempoProd : undefined;
  ...
  const collaboratorIds = collStats
  ...
  const allProductIds = Array.from(
    new Set([...Object.keys(productsMap), ...Object.keys(salesMap)])
  ).slice(0, 200);
  ...
  // ---------- Top by value ----------
  const topByValue = Object.entries(prodMap)
    .map(([product_id, qty]) => {
  ...
  // ---------- Produção por setor ----------
  const productionBySectorMap: Record<
    string,
    { qty_produced: number; value_br: number; hours_active: number }
  > = {};
  ...
  for (const r of productionRows) {
    ...
    if (!productionBySectorMap[sector]) {
      productionBySectorMap[sector] = {
        qty_produced: 0,
        value_br: 0,
        hours_active: 0,
      };
  ...
  const productionBySector = Object.entries(productionBySectorMap)
    .map(([sector, v]) => ({
      sector,
      qty_produced: v.qty_produced,
      value_br: v.value_br,
  ...
  for (const l of losses) {
    ...
    if (!refugoMap[userSector]) {
      refugoMap[userSector] = { qty_refugo: 0, value_br: 0 };
    }
  ...
  const refugoBySectorList = Object.entries(refugoMap)
    .map(([sector, v]) => ({
      sector,
      qty_refugo: v.qty_refugo,
      value_br: v.value_br,
    }))
  ...
  for (const r of productionRows) {
    ...
    if (!producedBySectorProductRaw[sector]) {
      producedBySectorProductRaw[sector] = {};
    }

    producedBySectorProductRaw[sector][pid] =
      (producedBySectorProductRaw[sector][pid] ?? 0) +
      Number(r.qty_produced ?? 0);
  }
  ...
  for (const sector of Object.keys(producedBySectorProductRaw)) {
    for (const pid of Object.keys(producedBySectorProductRaw[sector])) {
      producedBySectorProduct.push({
        ...
        value_br:
          producedBySectorProductRaw[sector][pid] *
          (productsMap[pid]?.standard_cost ?? 0),
      });
    }
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0 text-center sm:text-left">
          <h1 className="text-2xl font-bold leading-tight text-gray-900 sm:text-3xl dark:text-slate-100">
            Produtividade
          </h1>

          <p className="mx-auto max-w-prose text-sm text-gray-600 sm:mx-0 sm:text-base dark:text-slate-300">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <Card>
          ...
          <CardContent className="pt-0">
            <div className="text-xl font-bold sm:text-2xl">{totalPedidos}</div>
        ...
        <Card>
          ...
          <CardContent className="pt-0">
            <div className="text-xl font-bold sm:text-2xl">{ativos}</div>
            <p className="text-[11px] text-muted-foreground sm:text-xs">
              Em qualquer etapa
            </p>
          </CardContent>
        ...
        <Card>
          ...
          <CardContent className="pt-0">
            ...
            <p className="text-[11px] text-muted-foreground sm:text-xs">
              Baseado em {qtdComTempo} pedidos entregues
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="pt-0">
            <div className="text-xl font-bold sm:text-2xl">
              {formatMinutes(tempoMedioProducaoMin)}
            </div>
            <p className="text-[11px] text-muted-foreground sm:text-xs">
              Aceito → em_separação ({qtdComTempoProd})
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Usuários por quantidade produzida (últimos 30 dias)
            </CardDescription>
          </CardHeader>
                ...
                {productsMostProduced.map((p) => (
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-muted-foreground">
                        {p.product?.default_unit_label ?? "UN"}
                      </div>
                    </div>
                ...
                {forecasts.slice(0, 20).map((f) => (
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-muted-foreground">
                        {f.product?.default_unit_label ?? "UN"}
                      </div>
                    </div>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
            ...
            {productionBySector.length === 0 ? (
              ...
              <div className="space-y-2">
                {productionBySector.map((s) => (
                  ...
                  >
                    <div className="min-w-0">
                      <div className="break-words font-medium">{s.sector}</div>
                      <div className="text-xs text-muted-foreground">
                        {Number(s.qty_produced).toFixed(2)} UN
                      </div>
                    </div>

                    <div className="text-left sm:text-right">
                      <div className="font-semibold">
                        R$ {Number(s.value_br).toFixed(2)}
                      </div>
                      <div className="text-xs text-muted-foreground">
                        {Number(s.hours_active).toFixed(2)} h
                      </div>
                    </div>
                  </div>
                ))}
        ...
        <Card>
          <CardHeader>
            <CardTitle>Refugo por setor</CardTitle>
          </CardHeader>

          <CardContent>
            {refugoBySectorList.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                Nenhum refugo registrado no período.
              </p>
            ) : (
              <div className="space-y-2">
                {refugoBySectorList.map((r) => (
                  ...
                  >
                    <div className="min-w-0">
                      <div className="break-words font-medium">{r.sector}</div>
                      <div className="text-xs text-muted-foreground">
                        {Number(r.qty_refugo).toFixed(2)} UN
                      </div>
                    </div>

                    <div className="text-left sm:text-right">
                      <div className="font-semibold">
                        R$ {Number(r.value_br).toFixed(2)}
                      </div>
                    </div>
                  </div>
                ))}
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle>Produtos por giro (14d)</CardTitle>
          </CardHeader>

          <CardContent>
            {Object.entries(salesMap).slice(0, 20).length === 0 ? (
              <p className="text-sm text-muted-foreground">Nenhum dado.</p>
            ) : (
                  ...
                  .map(([pid, qty]) => (
                    ...
                    >
                      <div className="min-w-0 break-words text-sm">
                        {productsMap[pid]?.name ?? pid}
                      </div>
                      <div className="text-xs text-muted-foreground sm:text-right">
                        {Number(qty).toFixed(0)}
                      </div>
                    </li>
                  ))}
              </ul>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Produtos sazonais</CardTitle>
          </CardHeader>

          <CardContent>
            <p className="text-sm text-muted-foreground">
              Sazonalidade simples baseada em 8 semanas (média comparativa). Para
              análise avançada, usar série temporal.
            </p>
          </CardContent>
        </Card>
      </div>
