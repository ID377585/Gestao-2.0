...
export default function ContasAReceberPage() {
  ...
  async function loadData() {
    try {
      ...
      const [receivables, allCategories, bankStatusMap] = await Promise.all([
        listAccountsReceivable(),
        listFinancialCategories(),
        getBankStatusMap({ financeType: "receber" }),
      ]);

      const enriched: ReceivableRow[] = receivables.map((item) => ({
        ...item,
  ...
  const filteredItems = useMemo(() => {
    return items.filter((item) => {
      const statusOk =
        statusFilter === "todos" || item.statusRecebimento === statusFilter;
      ...
      return statusOk && categoryOk && bankOk && searchOk;
    });
  }, [items, statusFilter, categoryFilter, bankFilter, search]);
