  ...
  return (
    <div className="space-y-3">
      <div className="rounded-lg border">
        ...
        <div className="divide-y">
          {rows.map((row) => {
            ...
            const availableText = row.loadingStock ? "..." : row.available == null ? "-" : String(row.available);

            const qtyNum = Number(String(row.qty).replace(",", "."));
            const qtyInvalid = row.qty.trim().length > 0 && (!Number.isFinite(qtyNum) || qtyNum <= 0);
            const overStock = row.available != null && Number.isFinite(qtyNum) && qtyNum > row.available;
                ...
                <div className="grid grid-cols-12 gap-2 items-start">
                  <div className="col-span-5">
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        updateRow(row.rowId, { product_id, unit_label: unit_label || "", available: null, error: null });

                        setTimeout(() => {
                          refreshStock({ ...row, product_id, unit_label: unit_label || "" });
                        }, 0);
                      }}
                    >
                  ...
                  <div className="col-span-2">
                    <input
                      ...
                      onChange={(e) => updateRow(row.rowId, { unit_label: e.target.value.toUpperCase(), available: null, error: null })}
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="text-xs text-muted-foreground">Dica: selecione o produto para carregar o saldo automaticamente.</div>
      </div>
    </div>
  );
}
