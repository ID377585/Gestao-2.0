...
import { listProductCosts, type ProductCostInfo } from "@/lib/financeiro/product-costs";
...
function buildAlerts(params: {
  ...
  if (params.cmvPercentual > CMV_TARGET_PERCENT) {
    alerts.push({
      ...
      description: `O CMV está em ${formatPercent(params.cmvPercentual)}, acima da meta de ${formatPercent(CMV_TARGET_PERCENT)}.`,
      level: params.cmvPercentual >= CMV_TARGET_PERCENT + 5 ? "danger" : "warning",
    });
  }
  ...
  if (params.cmvAjustadoPercentual > CMV_TARGET_PERCENT) {
    alerts.push({
      ...
      description: `Considerando perdas valorizadas, o CMV ajustado está em ${formatPercent(params.cmvAjustadoPercentual)}.`,
      level:
        params.cmvAjustadoPercentual >= CMV_TARGET_PERCENT + 5 ? "danger" : "warning",
    });
  }
  ...
  if (params.cmvTeoricoPercentual > CMV_TARGET_PERCENT) {
    alerts.push({
      ...
      description: `A média de CMV teórico das fichas está em ${formatPercent(params.cmvTeoricoPercentual)}.`,
      level:
        params.cmvTeoricoPercentual >= CMV_TARGET_PERCENT + 5 ? "danger" : "warning",
    });
  }
  ...
  if (params.despesasPercentual > DESPESAS_TARGET_PERCENT) {
    alerts.push({
      ...
      description: `As despesas operacionais estão em ${formatPercent(params.despesasPercentual)}, acima da referência de ${formatPercent(DESPESAS_TARGET_PERCENT)}.`,
      level:
        params.despesasPercentual >= DESPESAS_TARGET_PERCENT + 5 ? "danger" : "warning",
    });
  }

  const ordered = [...params.lucroSerie].sort((a, b) => a.periodo.localeCompare(b.periodo));
  if (ordered.length >= 2) {
    ...
    if ((last?.lucroAjustado ?? 0) < (prev?.lucroAjustado ?? 0)) {
      alerts.push({
        ...
        description: "O lucro ajustado por perdas valorizadas caiu em relação ao período anterior.",
        level: "warning",
      });
    }
  }
  ...
  if (params.receitaLiquida > 0 && params.lucroLiquidoAjustado < 0) {
    alerts.push({
      ...
      description: "Considerando as perdas valorizadas, o resultado líquido ajustado fica negativo.",
      level: "danger",
    });
  }
  ...
  if (params.totalLosses >= LOSS_COUNT_WARNING || params.totalLossQty >= LOSS_QTY_WARNING) {
    ...
    alerts.push({
      ...
      level:
        params.totalLossQty >= LOSS_QTY_WARNING * 2 ? "danger" : "warning",
    });
  }
  ...
  if (params.perdasValorizadas > 0) {
    alerts.push({
      ...
      description: `Foi estimado impacto financeiro nas perdas de ${params.perdasValorizadas.toLocaleString("pt-BR", {
        style: "currency",
        currency: "BRL",
      })}.`,
      level: "info",
  ...
  if (params.technicalSheetsCount === 0) {
    alerts.push({
      ...
      description: "A DRE não encontrou fichas técnicas suficientes para calcular CMV teórico.",
      level: "warning",
    });
  }

  if (params.technicalSheetHighestCmv && params.technicalSheetHighestCmv.value > CMV_TARGET_PERCENT) {
    alerts.push({
      key: "ficha-cmv-critico",
      title: "Ficha técnica com CMV teórico crítico",
      description: `${params.technicalSheetHighestCmv.name} está com CMV teórico de ${formatPercent(params.technicalSheetHighestCmv.value)}.`,
      level:
  ...
  if (params.technicalSheetLowestMargin && params.technicalSheetLowestMargin.value <= 0) {
    alerts.push({
      key: "ficha-sem-margem",
      title: "Ficha com margem unitária zerada ou negativa",
      description: `${params.technicalSheetLowestMargin.name} está com margem unitária de ${params.technicalSheetLowestMargin.value.toLocaleString("pt-BR", {
        style: "currency",
        currency: "BRL",
      })}.`,
      level: "danger",
  ...
  if (params.varianceAveragePercent > 10) {
    alerts.push({
      ...
      description: `A variação média dos ingredientes com custo real está em ${formatPercent(params.varianceAveragePercent)} acima do teórico.`,
  ...
  if (params.varianceTopIngredient && params.varianceTopIngredient.value > 0) {
    alerts.push({
      ...
      description: `${params.varianceTopIngredient.name} acumula desvio positivo de ${params.varianceTopIngredient.value.toLocaleString("pt-BR", {
        style: "currency",
        currency: "BRL",
      })} sobre o teórico.`,
      level: "warning",
  ...
  if (params.varianceTopSheet && params.varianceTopSheet.value > 0) {
    alerts.push({
      ...
      description: `${params.varianceTopSheet.name} concentra desvio positivo de ${params.varianceTopSheet.value.toLocaleString("pt-BR", {
        style: "currency",
        currency: "BRL",
...
function buildInsights(params: {
  ...
  insights.push({
    key: "cmv-ajustado",
    label: `CMV ajustado com perdas valorizadas em ${formatPercent(params.cmvAjustadoPercentual)} da receita líquida.`,
  });

  insights.push({
    key: "cmv-teorico",
    label: `CMV teórico médio das fichas técnicas em ${formatPercent(params.cmvTeoricoPercentual)}.`,
  });

  insights.push({
    key: "variacao-media",
    label: `A variação média entre custo real e teórico dos ingredientes está em ${formatPercent(params.varianceAveragePercent)}.`,
  });
  ...
  if (params.perdasValorizadas > 0) {
    insights.push({
      key: "perdas-valor",
      label: `As perdas valorizadas somam ${params.perdasValorizadas.toLocaleString("pt-BR", {
        style: "currency",
        currency: "BRL",
      })}.`,
    });
  ...
  if (params.technicalSheetHighestCmv) {
    insights.push({
      key: "ficha-cmv-alto",
      label: `${params.technicalSheetHighestCmv.name} lidera o maior CMV teórico, com ${formatPercent(params.technicalSheetHighestCmv.value)}.`,
    });
  }
  ...
  if (params.maiorCentroCusto && params.receitaLiquida > 0) {
    ...
    insights.push({
      key: "centro-custo",
      label: `${params.maiorCentroCusto.name} concentra ${formatPercent(percentual)} da receita líquida em despesas.`,
    });
  }
...
export async function getDreDashboardData(
  ...
  const [
    ...
    listLosses(filters),
    listProductCosts(),
    getTechnicalSheetMetricsSummary(),
    getTechnicalSheetVarianceSummary(),
  ]);
  ...
  const lucroLiquidoAjustado =
    lucroBrutoAjustado - despesasOperacionais + resultadoFinanceiro - impostosResultado;
  ...
  const receitasVsDespesas = Array.from(byMonth.entries())
    ...
    .map(([periodo, values]) => ({
      ...
      lucroAjustado: values.receitas - values.despesas - values.perdasValorizadas,
    }));
  ...
  const cmvPercentual = receitaLiquida > 0 ? (cmv / receitaLiquida) * 100 : 0;
  const cmvAjustadoPercentual =
    receitaLiquida > 0 ? (cmvComPerdasValorizadas / receitaLiquida) * 100 : 0;
  const cmvTeoricoPercentual = technicalSheetSummary.averageTheoreticalCmvPercent;
  const despesasPercentual =
    receitaLiquida > 0 ? (despesasOperacionais / receitaLiquida) * 100 : 0;
  const margemBruta = receitaLiquida > 0 ? (lucroBruto / receitaLiquida) * 100 : 0;
  const margemBrutaAjustada =
    receitaLiquida > 0 ? (lucroBrutoAjustado / receitaLiquida) * 100 : 0;
  const margemLiquida =
    receitaLiquida > 0 ? (lucroLiquido / receitaLiquida) * 100 : 0;
  const margemLiquidaAjustada =
    receitaLiquida > 0 ? (lucroLiquidoAjustado / receitaLiquida) * 100 : 0;
  ...
  const varianceTopSheet = technicalSheetVarianceSummary.topSheetsByExposure[0];
  ...
  const alerts = buildAlerts({
    ...
    variancePositiveValue: technicalSheetVarianceSummary.totalPositiveVarianceValue,
  ...
  return {
    ...
    lines: [
      ...
      { key: "cmv", label: "(-) CMV / Serviços", value: -cmv },
      {
    ...
    charts: {
      composicaoDre: [
        ...
        { name: "CMV", value: cmv },
        { name: "Perdas valorizadas", value: perdasValorizadas },
      ...
      comparativoCmv: [
        { name: "CMV financeiro", value: cmvPercentual },
      ...
      fichasPorExposicao: technicalSheetVarianceSummary.topSheetsByExposure,
    },
  };
}