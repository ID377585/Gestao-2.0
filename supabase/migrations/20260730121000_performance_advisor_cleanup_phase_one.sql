...
do $$
begin
  -- Several operational tables predate the tracked migration chain in legacy
  -- Production. Guard each advisory index by the actual column as well as the
  -- relation, so a clean replay remains valid without inventing legacy columns.
  ...
  if to_regclass('public.kitchen_checklist_runs') is not null
     and exists (
       select 1 from information_schema.columns
       where table_schema = 'public' and table_name = 'kitchen_checklist_runs' and column_name = 'template_id'
     ) then
  ...
  if to_regclass('public.kitchen_checklist_runs') is not null
     and exists (
       select 1 from information_schema.columns
       where table_schema = 'public' and table_name = 'kitchen_checklist_runs' and column_name = 'opened_by'
     ) then
  ...
  if to_regclass('public.kitchen_checklist_runs') is not null
     and exists (
       select 1 from information_schema.columns
       where table_schema = 'public' and table_name = 'kitchen_checklist_runs' and column_name = 'completed_by'
     ) then
  ...
  if to_regclass('public.kitchen_checklist_run_items') is not null
     and exists (
       select 1 from information_schema.columns
       where table_schema = 'public' and table_name = 'kitchen_checklist_run_items' and column_name = 'template_item_id'
     ) then
  ...
  if to_regclass('public.kitchen_checklist_run_items') is not null
     and exists (
       select 1 from information_schema.columns
       where table_schema = 'public' and table_name = 'kitchen_checklist_run_items' and column_name = 'checked_by'
     ) then
  ...
  if to_regclass('public.kitchen_checklist_templates') is not null
     and exists (
       select 1 from information_schema.columns
       where table_schema = 'public' and table_name = 'kitchen_checklist_templates' and column_name = 'created_by'
     ) then
  ...
  if to_regclass('public.product_nutrition_facts') is not null
     and exists (
       select 1 from information_schema.columns
       where table_schema = 'public' and table_name = 'product_nutrition_facts' and column_name = 'created_by'
     ) then
  ...
  if to_regclass('public.sales_price_benchmarks') is not null
     and exists (
       select 1 from information_schema.columns
       where table_schema = 'public' and table_name = 'sales_price_benchmarks' and column_name = 'created_by'
     ) then
  ...
  if to_regclass('public.technical_sheet_nutrition_snapshots') is not null
     and exists (
       select 1 from information_schema.columns
       where table_schema = 'public' and table_name = 'technical_sheet_nutrition_snapshots' and column_name = 'created_by'
     ) then
