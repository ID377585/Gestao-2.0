...
-- Keep the older two-argument signature and its default available for existing
-- callers. PostgreSQL does not allow CREATE OR REPLACE to remove an existing
-- parameter default, so the historical contract must be preserved explicitly.
create or replace function public.claim_app_jobs(
  p_worker_id text,
