-- Harden nutrition notification creation without moving business modules.
-- The RPC remains callable by authenticated users, but authorization is
-- enforced inside the database as a second barrier behind Server Actions.
...
create index if not exists nutrition_notifications_target_user_id_idx
  on public.nutrition_notifications(target_user_id);

create index if not exists nutrition_notifications_created_by_idx
  on public.nutrition_notifications(created_by);

comment on function public.enqueue_nutrition_notification(
  uuid, text, text, text, text, text, uuid, uuid, timestamptz, text, jsonb
) is 'Creates tenant-scoped nutrition notifications after module authorization, target membership validation and bounded input checks.';
