...
begin
  if p_establishment_id is null then
    raise exception 'establishment_id is required' using errcode = '22023';
  end if;

  if nullif(trim(coalesce(p_type, '')), '') is null or length(p_type) > 100 then
    raise exception 'invalid notification type' using errcode = '22023';
  end if;

  if nullif(trim(coalesce(p_title, '')), '') is null or length(p_title) > 240 then
    raise exception 'invalid notification title' using errcode = '22023';
  end if;

  if nullif(trim(coalesce(p_message, '')), '') is null or length(p_message) > 4000 then
    raise exception 'invalid notification message' using errcode = '22023';
  end if;
  ...
  if v_request_role <> 'service_role' then
    ...
    if v_membership_role is null then
      raise exception 'establishment access denied' using errcode = '42501';
    end if;

    select ump.can_access into v_explicit_permission
    ...
    if not v_can_access then
      raise exception 'nutrition module access denied' using errcode = '42501';
    end if;
  end if;
  ...
  if p_target_user_id is not null and not exists (
    ...
    raise exception 'notification target is outside establishment' using errcode = '42501';
  end if;

  insert into public.nutrition_notifications (
    establishment_id,target_user_id,notification_type,priority,title,message,
    resource_type,resource_id,due_at,dedupe_key,payload,created_by
  ) values (
    p_establishment_id,p_target_user_id,trim(p_type),
    case when p_priority in ('low','normal','high','critical') then p_priority else 'normal' end,
    trim(p_title),trim(p_message),nullif(trim(coalesce(p_resource_type,'')),''),
    p_resource_id,p_due_at,nullif(trim(coalesce(p_dedupe_key,'')),''),
    coalesce(p_payload,'{}'::jsonb),v_uid
  )
